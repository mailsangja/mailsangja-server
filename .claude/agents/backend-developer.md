...
6. 구현을 마친 뒤에는 `facade-service-test-conventions.md`를 다시 읽고, 관련 테스트를 작성 또는 보강합니다.
7. 관련 모듈 테스트를 실행하고, `jacocoTestReport` 및 `jacocoTestCoverageVerification`으로 `facade`/`service` 커버리지 70% 충족 여부를 확인합니다.
