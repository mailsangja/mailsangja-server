...
class MessagePermanentlyDeletedHistoryEventHandlerTest {
    ...
    private MessagePermanentlyDeletedHistoryEventHandler handler;

    @BeforeEach
    void setUp() {
        handler = new MessagePermanentlyDeletedHistoryEventHandler(mailAccountQueryService, gmailHistoryDeleteApplyCommandService);
    }

    @Test
    void handle_동기화가능계정을조회해영구삭제를위임한다() {
        ...
        assertEquals(GmailHistoryEventType.MESSAGE_PERMANENTLY_DELETED, handler.supports());
        verify(gmailHistoryDeleteApplyCommandService).applyMessagePermanentlyDeleted(mailAccount, event);
    }
}
