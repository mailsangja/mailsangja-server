...
public interface InboxControllerDocs {
    ...
    @Operation(
            summary = "메일 스레드 읽음 처리",
            description = "특정 스레드를 읽음 처리합니다. 대상 스레드는 INBOUND, OUTBOUND 모두 가능합니다.",
            security = @SecurityRequirement(name = "cookieAuth")
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "스레드 읽음 처리 성공"),
            @ApiResponse(responseCode = "401", description = "인증 필요",
                    content = @Content(schema = @Schema(hidden = true))),
            @ApiResponse(responseCode = "403", description = "스레드 접근 권한 없음",
                    content = @Content(schema = @Schema(hidden = true))),
    ...
    @Operation(
            summary = "스레드 별표 토글",
            description = "특정 스레드의 별표 상태를 토글합니다. 별표가 없으면 추가하고, 있으면 제거합니다.",
            security = @SecurityRequirement(name = "cookieAuth")
    )
    @ApiResponses({
    ...
    ResponseEntity<Void> toggleStar(
            @Parameter(hidden = true) @AuthUser User user,
            @Parameter(description = "스레드 내부 ID", required = true)
            @PathVariable UUID threadId
    );
